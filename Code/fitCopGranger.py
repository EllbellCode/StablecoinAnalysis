...
"""
Copula Granger Causality key functions
"""

# Calculates the gaussian kernel weights
...
# Uses Silverman's rule to calculate bandwidth
...
# Calculates the conditional marginal distributions (CMDs) for Y
# Given history of Y, what is the rank of current Y?
def yCMD(y_next, y_lags, h, query_y_next=None, query_y_lags=None):

    if query_y_lags is None: 
        query_y_lags, query_y_next = y_lags, y_next
...
# Calculates the conditional marginal distributions (CMDs) for X conditional on Y
# Given history of Y, what is the rank of current X?
def xCMD(x_lags, y_lags, h, query_x_lags=None, query_y_lags=None):

    T, m = x_lags.shape

    if query_y_lags is None: 
        query_y_lags, query_x_lags = y_lags, x_lags
...
# Bernstein Copula Density Estimation
def calcBernstein(u, v, m=10):
    ...
    def get_B(points): 
        return np.exp((alpha - 1) * np.log(points[:, None]) + (beta - 1) * np.log(1 - points[:, None]) + log_norm)
    
    return np.clip(np.einsum('tm,tm->t', get_B(u) @ W, get_B(v)), 1e-12, None)

# Calculates the Granger Causality statistic
...
# Generates the synthetic GC distribution
...
if __name__ == "__main__":
    ...
    # Run CGC Tests for each factor pair
    results, nulls = [], []
    print("-" * 60)
