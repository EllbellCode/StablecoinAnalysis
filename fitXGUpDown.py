...
OUTPUT_DIR = Path("Results/ML/XG/UpDown")
PLOT_DIR = Path("Plots/ML/Gain/XG/UpDown")
...
VOLATILITY = "Delta_LogGK"       # Original Volatility Measure
UPSIDE_VAR = "Upside_Vol"        # New Upside Volatility Measure
DOWNSIDE_VAR = "Downside_Vol"    # New Downside Volatility Measure

#   Stable_Volatility, Stable_Upside_Vol, Stable_Downside_Vol, Stable_Volume
#   Crypto_Volatility, Crypto_Upside_Vol, Crypto_Downside_Vol, Crypto_Returns

SOURCE_FACTOR = "Stable_Upside_Vol"     
TARGET_FACTOR = "Crypto_Upside_Vol"  
...
def diebold_mariano_test(real, pred1, pred2, h=1, metric='mse'):
    ...
    if metric == 'mse':
        ...
        e1 = (real_sign != p1_sign).astype(int) 
        e2 = (real_sign != p2_sign).astype(int) 
    else:
...
def generate_factors_window(data_dict, current_end_date):
    ...
    # --- MODIFIED: ADD UPSIDE AND DOWNSIDE VOLATILITY FACTORS ---
    f["Stable_Upside_Vol"] = calculate_pca_for_window(stablecoins, UPSIDE_VAR, data_dict, "PC1_Stable_Upside_Vol", current_end_date)
    f["Stable_Downside_Vol"] = calculate_pca_for_window(stablecoins, DOWNSIDE_VAR, data_dict, "PC1_Stable_Downside_Vol", current_end_date)
    
    f["Crypto_Upside_Vol"] = calculate_pca_for_window(cryptos, UPSIDE_VAR, data_dict, "PC1_Crypto_Upside_Vol", current_end_date)
    f["Crypto_Downside_Vol"] = calculate_pca_for_window(cryptos, DOWNSIDE_VAR, data_dict, "PC1_Crypto_Downside_Vol", current_end_date)
...
for current_date in tqdm(test_dates):
    ...
    X_bench_v, y_bench_v, X_chall_v, y_chall_v = create_vol_on_garch_features(
        y_vol_series=u_vol_exp, u_shock_series=u_target_exp, v_shock_series=v_resid_exp, v_vol_series=v_vol_exp, specific_lags=specific_lags
    )
    ...
    if not X_bench_v.empty and not X_chall_v.empty:
        ...
        # Note: 'target_vol' isn't needed for prediction X construction, but we need dummy placeholder
        X_b_next_v, _, X_c_next_v, _ = create_vol_on_garch_features(
            y_vol_series=u_vol_temp, u_shock_series=u_target_temp, v_shock_series=v_resid_temp, v_vol_series=v_vol_temp, specific_lags=specific_lags
        )
        ...
        oos_predictions_vol.append({
            'Date': current_date,
            'Actual': vol_next, # Actual Volatility is often proxied by next GARCH vol or realizied vol? Using GARCH forecast as proxy here or actual realizied? 
                                # In this script logic, it seems we compare against "Actual" volatility from GARCH fit? 
                                # Actually, standard practice compares against Realized Vol. 
                                # But here, let's keep consistent with your original script which likely compares to conditional vol or similar.
            'Pred_Bench': pred_bench_v,
            'Pred_Chall': pred_chall_v
        })
...
if oos_predictions_garch:
    ...
    dm_stat_g, dm_p_g = diebold_mariano_test(df_res_g['Actual'], df_res_g['Pred_Bench'], df_res_g['Pred_Chall'], metric='mse')
    
    # Directional Accuracy (PT Test)
    ...
    dm_stat_dir, dm_p_dir = diebold_mariano_test(df_res_g['Actual'], df_res_g['Pred_Bench'], df_res_g['Pred_Chall'], metric='direction')

    print(f"\n[TEST 1: GARCH Model (Return Prediction)]")
...
if oos_predictions_vol:
    df_res_v = pd.DataFrame(oos_predictions_vol).set_index('Date')
    mse_b_v = mean_squared_error(df_res_v['Actual'], df_res_v['Pred_Bench'])
    mse_c_v = mean_squared_error(df_res_v['Actual'], df_res_v['Pred_Chall'])
    
    dm_stat_v, dm_p_v = diebold_mariano_test(df_res_v['Actual'], df_res_v['Pred_Bench'], df_res_v['Pred_Chall'], metric='mse')
...
if summary_rows:
    ...
    cols = [
        ...
        'MSE_Benchmark', 'MSE_Challenger', 'DM_MSE_Stat', 'DM_MSE_P_Value',
        'Benchmark_Directional_Acc', 'Challenger_Directional_Acc', 'DM_Directional_Stat', 'DM_Directional_P_Value'
    ]
    summary_df = summary_df[cols]
    
    print("\nSummary Results:")
    print(summary_df)
    summary_df.to_csv(OUTPUT_DIR / f"Summary_{SOURCE_FACTOR}_to_{TARGET_FACTOR}.csv", index=False)