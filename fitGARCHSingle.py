"""
Fits a variable of a single stablecoin of lags up to max lags in exogenous 
"""
...
# Iterate over all cryptos, variables, models, and stablecoins
    ...
    for var in variables:
        for model in models:
            for sc_name, sc_data in stablecoin_dfs.items():
                # Join crypto data with this single stablecoin
                data = df.join(sc_data[[var]].rename(columns={var: f'{sc_name}_{var}'}), how='inner')
                ...
                results.append({
                    ...
                    **{f'Mean_{k}': v for k, v in r0.params.filter(like='mu').items()},
                    **{f'Pval_{k}': v for k, v in r0.pvalues.filter(like='mu').items()}
                })
                ...
                results.append({
                    ...
                    'Variable': var,
                    'Stablecoin': sc_name,
                    'Model': f'{model} + AR(1) + {sc_name} lags 1-{max_lags}',
...
# Print crypto and significant lags
...
"""
Using Lag 1 eGARCH:
- DAI RV is significant for BNB, BTC, and ETH
- USDT RV is significant for ETH
- USDC Volume is significant for XRP and BTC
- USDT Volume for XRP
- DAI volume for BNB

Using Lag 1 GARCH:
- DAI RV is significant for BNB and ETH
- USDC Volume in ETH and XRP
