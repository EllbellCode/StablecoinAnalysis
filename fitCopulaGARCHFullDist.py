...
def plot_pit_analysis(results_df, pit_col, model_name):
    """
    Analyzes the PIT values for a given model.
    Plots a histogram and Q-Q plot, and runs uniformity/independence tests.
    """
    print(f"\n--- PIT Analysis: {model_name} ---")
    pit_data = results_df[pit_col].dropna()
    pit_data = np.clip(pit_data, 1e-6, 1 - 1e-6) # Clip for stability
    n_bins = max(20, int(len(pit_data) / 20)) # Binning rule

    # 1. Test for Uniformity (Kolmogorov-Smirnov)
    # Null Hypothesis: The data is from a Uniform(0,1) distribution.
    # A high p-value (e.g., > 0.05) is GOOD. It means we fail to reject the null.
    ks_stat, ks_pvalue = stats.kstest(pit_data, 'uniform')
    print(f"Kolmogorov-Smirnov Test (Uniformity):")
    print(f"  Stat={ks_stat:.4f}, p-value={ks_pvalue:.4f}")
    if ks_pvalue < 0.05:
        print("  -> RESULT: Rejects uniformity (Bad). Density forecasts are mis-specified.")
    else:
        print("  -> RESULT: Fails to reject uniformity (Good).")

    # 2. Test for Independence (Ljung-Box)
    # Null Hypothesis: The data is independently distributed (no autocorrelation).
    # A high p-value (e.g., > 0.05) is GOOD.
    pit_centered = pit_data - 0.5 # Center for testing
    lb_test = acorr_ljungbox(pit_centered, lags=[10], return_df=False)
    lb_pvalue = lb_test['lb_pvalue'].iloc[0]
    print(f"Ljung-Box Test (Independence, lag 10):")
    print(f"  p-value={lb_pvalue:.4f}")
    if lb_pvalue < 0.05:
        print("  -> RESULT: Rejects independence (Bad). Forecasts show autocorrelation.")
    else:
        print("  -> RESULT: Fails to reject independence (Good).")

    # 3. Plotting
    fig, axes = plt.subplots(1, 2, figsize=(15, 6))
    fig.suptitle(f"Probability Integral Transform (PIT) Analysis for {model_name}", fontsize=16)

    # Histogram
    axes[0].hist(pit_data, bins=n_bins, density=True, alpha=0.7, label=f'PIT Histogram (Bins={n_bins})')
    ...
    # Q-Q Plot
    qqplot(pit_data, dist=stats.uniform, line='45', ax=axes[1], fit=True)
...
def run_backtest(df_full, crypto_col, stable_col, n_train, n_test):
    ...
    for i in tqdm(range(n_test)):
        ...
        # --- ADDED: Calculate Benchmark PIT ---
        # 1. Scale actual outcome just like the training data
        actual_scaled = actual_outcome * 100 
        # 2. Standardize actual outcome using forecasted parameters
        bench_z_actual = (actual_scaled - bench_mu_val) / bench_sigma_val
        # 3. Apply the forecasted CDF to get the PIT value
        pit_benchmark = bench_dist.cdf(bench_z_actual, bench_dist_params)
        pit_benchmarks.append(pit_benchmark)
        ...
        challenger_vaR = np.percentile(returns_sim, 5)
        challenger_vaR_5pct.append(challenger_vaR)

        n_sim = len(returns_sim)
        pit_challenger = (returns_sim < actual_outcome).sum() / n_sim
        pit_challengers.append(pit_challenger)
    ...
    results_df = pd.DataFrame({
        ...
        'PIT_Benchmark': pit_benchmarks,
        'PIT_Challenger': pit_challengers
    }, index=df_full.index[n_train:n_train + n_test])

    initial_rows = len(results_df)
    subset_cols = ['Benchmark_VaR', 'Challenger_VaR', 'PIT_Benchmark', 'PIT_Challenger']
    results_df.dropna(subset=subset_cols, inplace=True)
...
if __name__ == "__main__":
    ...
    # --- ADDED: Run PIT Analysis ---
    plot_pit_analysis(results, 'PIT_Benchmark', 'Benchmark (EGARCH)')
    plot_pit_analysis(results, 'PIT_Challenger', 'Challenger (Copula-EGARCH)')